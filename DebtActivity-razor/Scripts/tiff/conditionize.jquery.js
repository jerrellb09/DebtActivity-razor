  ...
  $.fn.conditionize = function(options) {
    ...
    $.fn.showOrHide = function(listenTo, listenFor, operator, $section) {
      if ($(listenTo).is('select, input[type=text]') && $.fn.eval($(listenTo).val(), listenFor, operator)) {
        ...
        $section.slideUp();
        $section.find('select, input, textarea').each(function(){ // dg added textarea
            if ( ($(this).attr('type')=='radio') || ($(this).attr('type')=='checkbox') ) {
                $(this).prop('checked', false).trigger('change'); // dg unsets any checked input
            }
            else{
                if (!$(this).attr('type') =='submit') { // dg i added this  'if' to exclude submit buttons
                    $(this).val('').trigger('change');  // dg this causes button value to be set to ""
                }
            }
        });
      }
